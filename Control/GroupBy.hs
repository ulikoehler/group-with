module Control.GroupBy where
-----------------------------------------------------------------------------
-- |
-- Module      :  Control.GroupBy
...
groupBy :: (Ord b) => (a -> b) -> [a] -> MultiMap b a
groupBy f xs = Map.fromListWith (++) [(f x, [x]) | x <- xs]
...
--   identifier
groupByMultiple :: (Ord b) => (a -> [b]) -> [a] -> MultiMap b a
groupByMultiple f xs = 
